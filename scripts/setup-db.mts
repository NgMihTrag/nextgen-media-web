...
import { user, session, account, verification, portfolioProjects, testimonials, teamMembers, siteStats } from '@/lib/db/schema.js'
...
async function setupDatabase() {
  try {
    ...
    const existingAdmin = await db.query.user.findFirst({
      where: (u) => sql`${u.email} = 'nextgenmedia868@gmail.com'`
    }).catch(() => null)
